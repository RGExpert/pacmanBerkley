...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState: pacman.GameState):
        ...
        self._expanded = 0 # DO NOT CHANGE; Number of search nodes expandeds
...
def wallCounting(position, destination, walls):
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    position, notVisited=state
    
    min=999999
    for corner in corners:
        if corner in notVisited:
            manhattanHeuristicVal=util.manhattanDistance(corner,position)
            wallCountingVal=wallCounting(position,corner,walls)

            if manhattanHeuristicVal < wallCountingVal:
                distanceAprox=manhattanHeuristicVal + wallCountingVal
            else:
                distanceAprox=manhattanHeuristicVal

            if distanceAprox < min:
                min=distanceAprox
...
def find_clusters(foodList):
    ...
    def is_valid_position(position):
        return position in foodList and position not in visited

    def dfs(position, cluster):
        ...
        for dr, dc in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
            ...
            if is_valid_position(new_position):
                dfs(new_position, cluster)

    for position in foodList:
        if is_valid_position(position):
            cluster = []
            dfs(position, cluster)
            if cluster:
                clusters.append(tuple(cluster))  # Convert the list to a tuple before appending

    return tuple(map(tuple, clusters))  # Convert the list of tuples to a tuple of tuples
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if nextFood[nextx][nexty]==True:
                    new_tuple = tuple(
                        tuple(x for x in sub_tuple if x != (nextx, nexty)) 
                        for sub_tuple in clusters 
                        if any(x != (nextx, nexty) for x in sub_tuple))
                else:
                    new_tuple=clusters
...
def isNextTo(position1, position2):
    x1, y1 = position1
    x2, y2 = position2
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for cluster in clusters:
        ...
        smallest_wall_count=9999
        for elem in cluster:
            if util.manhattanDistance(elem,position) < distance:
                distance=util.manhattanDistance(elem,position)
                elem_to_remove=elem
            
            if wallCounting(position,elem,walls) < smallest_wall_count:
                smallest_wall_count=wallCounting(position,elem,walls)

        for i in range(len(cluster)):
            if cluster[i]!=elem_to_remove:
                distance+=1
        
        if max_distance >= smallest_wall_count:
            distance+=smallest_wall_count        
